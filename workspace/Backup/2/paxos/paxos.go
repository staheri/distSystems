		...
		for ! px.isDecided[seq] {
			...
			//	px.PrintStat()
				...
				if reply.Err == "OK" {
					...
					if reply.AcceptedPreNum > highestAcceptedSeen { // To be Compeleted
						//fmt.Printf("from %d REPLY.AcceptPreNum : %d **** highestAcceptedSeen : %d\n",idx,reply.AcceptedPreNum, highestAcceptedSeen)
						px.mu.Lock()
						highestAcceptedSeen = reply.AcceptedPreNum
						//fmt.Printf("%d::%d  VALUE CHANGED \n",px.me,seq)
						t1 := px.instPre[seq]
						t1.chooseN = reply.AcceptedPreNum
						//fmt.Printf("%d,%d :: VAL     NNNNN : %d \n",px.me,seq,t1.chooseN)
					...
					if (px.instPre[seq].chooseN < reply.AcceptedPreNum) {
						t1 := px.instPre[seq]
						t1.chooseN = reply.AcceptedPreNum
						//fmt.Printf("%d,%d :: REJJJ     NNNNN : %d \n",px.me,seq,t1.chooseN)
						px.instPre[seq] = t1
						//t1 = ProposerInstance{}
						//fmt.Printf("%d,%d :: KIRRRRRRRRRRRRRRRRRRRRR : %d \n",px.me,seq,px.instPre[seq].chooseN)
					} 
					// else {
						// if reply.AcceptedPreNum == 0 {
							// fmt.Printf("IGNORE ")
						// }
						// fmt.Printf("%d,%d KOSSSSSSSSSSSSSSSSS choose : %d , preNum : %d , idx : %d\n",px.me,seq,px.instPre[seq].chooseN,reply.AcceptedPreNum,idx)
					// }
					// if px.instPre[seq].chooseN == 0 {
			...
			for i := 0; i < len(px.peers); i++ {
				...
				for tmp := 0 ; tmp < 10 ; tmp++ {
					...
					if ok == false {
						//fmt.Printf("%d,%d : %d : CALL FAILLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL \n",px.me,seq,i)
						fmt.Errorf("CALL Decided failed")
					} else {
						break
					}
			...
			//fmt.Printf("%d,%d :: KIRRRRRRRRRRRRRRRRRRRRR \n",px.me,seq)
		}
	}()
	}
}
