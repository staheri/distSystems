...
func (ck *Clerk) Get(key string) string {
	...
	for {
		...
		if ok == false {
			//fmt.Printf("GET CALL FAILED , key: %d, xid: %d\n",args.Key,args.XID)
			//fmt.Errorf("CALL GET failed")
			sel = (sel + 1) % len(ck.servers)
		} else {
			if reply.Err == "OK" {
				//fmt.Printf("GET REPLY OK , key: %d, xid: %d\n",args.Key,args.XID)
				break
			} else {
...
func (ck *Clerk) PutAppend(key string, value string, oper string) {
	...
	sel := 0
	for {
		//fmt.Printf("PUT/APPEND CALL sel: %d  , key: %d, xid: %d\n",sel,args.Key,args.XID)
		ok := call(ck.servers[sel], "KVPaxos.PutAppend", args, &reply)
		if ok == false {
			//fmt.Printf("PUT/APPEND CALL FAILED  , key: %d, xid: %d\n",args.Key,args.XID)
			fmt.Errorf("CALL GET failed")
			sel = (sel + 1) % len(ck.servers)
		} else {
			if reply.Err == "OK" {
				//fmt.Printf("PUT/APPEND REPLY OK , key: %d, xid: %d\n",args.Key,args.XID)
				break
			} else {
